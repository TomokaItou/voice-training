// Injects the large static application shell before the rest of the app scripts run.
(function () {
  const appMarkup = String.raw`    <section class="mode-launcher" id="modeLauncher">
      <div class="mode-launcher-card">
        ...
        <section class="mira-hero mira-state-idle" aria-label="Mira 声音导航">
          ...
          <div class="voice-navigation-deck">
            <div class="mira-dialogue">
              ...
              <h1>今天练 30 秒。</h1>
              <p>不用一上来就唱歌。先完成一个很小的任务，Mira 陪你慢慢靠近开口。</p>
              <div class="mira-actions">
                <button id="startTodayTrainingButton" class="hall-primary-action" type="button">今天练 30 秒</button>
                <button id="miraSongButton" class="secondary" type="button">选择目标歌曲</button>
              </div>
              <div class="beginner-quest-preview" id="beginnerQuestPreview">
                <span id="beginnerQuestPreviewLevel">Lv0</span>
                <div>
                  <strong id="beginnerQuestPreviewTitle">今天不用唱歌。先听 10 秒就可以。</strong>
                  <small id="beginnerQuestPreviewMeta">约 30 秒 · 完成就算赢</small>
                </div>
              </div>
            </div>

            <section class="target-navigation-card" aria-label="目标声音导航卡">
              <div class="navigation-card-header">
                <span class="game-label">当前路线</span>
                <strong>等待目标声音</strong>
              </div>
              <dl class="navigation-data">
                <div>
                  <dt>目标声音</dt>
                  <dd>未选择</dd>
                </div>
              ...
              <p>一轮结束后，回听录音，再进入 Fix One Thing 做 30 秒对照练习。</p>
            </section>

            <section class="voice-compass-card" aria-label="声音罗盘">
              <div>
                <span class="game-label">闭环判断</span>
                <strong>完成跟唱后生成声音坐标</strong>
              </div>
              <div class="voice-compass" aria-hidden="true">
                <span class="compass-current">●</span>
                <span class="compass-target">★</span>
                <span class="compass-line"></span>
              </div>
              <p>等待 Mira 判断：音高、节奏、气息或共鸣，今天只先修一项。</p>
            </section>
          </div>
        </section>

        <section class="daily-challenge-card" id="dailyChallengeEntry">
          <div>
        ...
        <section class="daily-challenge-panel" id="dailyChallengePanel" hidden>
        ...
        <section class="beginner-practice-panel" id="beginnerPracticePanel" hidden>
            ...
            <section class="song-search-card route-panel route-song-card" aria-label="选择目标歌曲">
              ...
              <form id="songSearchForm" class="song-search-form">
                <input
                  ...
                  required
                />
                <button id="songSearchButton" type="submit" class="secondary">搜索</button>
              </form>
              <p id="songSearchStatus" class="song-search-status">找一首今天想靠近的歌，先把目标定住。</p>
        ...
        <details class="game-player-details">
          <summary>
            <span>
              <strong>成长记录</strong>
        ...
        <details class="launcher-secondary launcher-tools-details">
          <summary>
            <span>
              <strong>高级工具</strong>
        ...
        <details class="readiness-card" id="readinessCard">
    ...
    <section class="library-page" id="libraryPage" hidden>
    ...
    <main class="app" id="appWindow" hidden>
      <header>
      ...
      <section class="practice-controls" id="practiceControls">
        <div class="controls-main">
          <button id="startButton" data-task-control="curve score breath range spectrogram volume formants memory rhythm action">开始练习</button>
          <button id="pauseButton" class="secondary" data-task-control="curve score breath range spectrogram volume formants memory rhythm action" disabled>暂停</button>
          <button id="stopButton" data-task-control="curve score breath range spectrogram volume formants memory rhythm action" disabled>停止</button>
    ...
    <aside class="sidebar" id="sidebar" aria-hidden="true" hidden>
      <div class="sidebar-header">
        <h2>设置</h2>
        <button id="closeSidebarButton" class="secondary sidebar-close-button" type="button">关闭</button>
      </div>
      <details class="sidebar-card advanced-settings" data-task-section="curve spectrogram volume formants">
        <summary>
          <span>高级参数</span>
      ...
      <div class="sidebar-card" data-task-section="curve">
        <h2>音高算法</h2>
        <label class="field">
      ...
      <div class="sidebar-card" data-task-section="curve spectrogram volume formants">
        <h2>音高刻度</h2>
        <label class="field">
