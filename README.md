...
18. 出气量测量：进入该模式后建议先点击“校准环境”，保持 2 秒安静，软件会记录当前环境噪声底；之后再对准麦克风平稳吹气，可看到扣除环境底噪后的出气评分、气流强度、3kHz 以上高频气声、稳定度和持续时间。
...
- **出气量测量 CSV**：在出气量测量模式下会导出最近一段出气记录。
  - CSV 列包含 `timestampMs`、`breathScorePercent`、`effortPercent`、`highFrequencyBreathPercent`、`stabilityPercent`、`durationSeconds`。
